...
final class MetricsDeletionMiddlewareTest extends TestCase
{
    public function testFullOperationEmitsDurationsAndCounters(): void
    {
        ...
        $mw->beforeDetachRelations('ParentNs\\Parent', 'ChildNs\\Child', [1, 2], $relation, $root);
        $mw->afterDetachRelations('ParentNs\\Parent', 'ChildNs\\Child', [1, 2], $relation, $root);
        $mw->beforeDeleteChildren('ChildNs\\Child', [1, 2, 3], $root);
        $mw->afterDeleteChildren('ChildNs\\Child', [1, 2, 3], $root);
        $mw->beforeDeleteRoot($root);
        $mw->afterDeleteRoot($root);
    ...
    public function testFlushPendingReportsIncompletePhases(): void
    {
        ...
        $mw->beforeDeleteRoot(new ScalarParentFixture());
        $mw->flushPending();
